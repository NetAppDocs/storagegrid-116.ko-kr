...
IMPORTANT: 복구 중에 스토리지 노드를 재부팅하지 말고, 실패한 스토리지 볼륨을 다시 포맷하고 객체 메타데이터를 복구하기 위해 'sn-recovery-postinstall.sh'(4단계)를 실행하십시오. 'sn-recovery-postinstall.sh'가 완료되기 전에 스토리지 노드를 재부팅하면 서비스를 시작할 때 오류가 발생하여 StorageGRID 어플라이언스 노드가 유지보수 모드를 종료합니다.
...
. 'sn-recovery-postinstall.sh' 스크립트가 실행되면 Grid Manager에서 복구 페이지를 모니터링합니다.
+
...
. 컴퓨팅 컨트롤러의 IP 주소를 사용하여 https://Controller_IP:8443` 를 입력하여 StorageGRID 어플라이언스 설치 프로그램의 모니터 설치 페이지로 돌아갑니다.
+
...
xref:restoring-object-data-to-storage-volume-for-appliance.adoc[어플라이언스의 스토리지 볼륨에 오브젝트 데이터를 복원합니다]
