...
[role="lead"]
하나 이상의 실패한 스토리지 볼륨을 다시 포맷하기 위해 'sn-recovery-postinstall.sh' 스크립트가 필요한 경우 다른 스토리지 노드 및 아카이브 노드에서 포맷된 스토리지 볼륨으로 객체 데이터를 복원해야 합니다. 하나 이상의 스토리지 볼륨을 다시 포맷하지 않으면 이러한 단계가 필요하지 않습니다.
