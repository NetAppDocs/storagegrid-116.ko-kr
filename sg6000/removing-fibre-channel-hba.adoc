...
.이 작업에 대해
서비스 중단을 방지하려면 Fibre Channel HBA 교체를 시작하기 전에 다른 모든 스토리지 노드가 그리드에 연결되어 있는지 확인하거나 서비스 중단이 정상적으로 예상되는 예약된 유지 보수 기간 동안 어댑터를 교체하십시오. 에 대한 정보를 참조하십시오 link:../monitor/monitor-node-connection-states.html["노드 연결 상태를 모니터링합니다"].
