...
. [[CHANGE_LINK_CONFIGURATION_sg5600, START=2]]링크 구성을 원하는 대로 변경합니다.
+
...
+
VLAN 설정을 변경한 경우 어플라이언스의 서브넷이 변경되었을 수 있습니다. 어플라이언스의 IP 주소를 변경해야 하는 경우 에 따르십시오 xref:setting-ip-configuration-sg5600.adoc[IP 구성을 설정합니다] 지침.

. StorageGRID 어플라이언스 설치 프로그램에서 * 네트워킹 구성 * > * 핑 테스트 * 를 선택합니다.
. Ping 테스트 도구를 사용하여 에서 변경한 링크 구성 변경의 영향을 받을 수 있는 네트워크의 IP 주소에 대한 연결을 확인합니다 <<change_link_configuration_sg5600,링크 구성을 변경합니다>> 단계.
+
